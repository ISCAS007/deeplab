...
def randomly_scale_image_and_label(image, label=None, edge=None, scale=1.0):
    ...
    if label is not None:
        label = tf.squeeze(tf.image.resize_nearest_neighbor(
            ...
            align_corners=True), [0])
    if edge is not None:
        edge = tf.squeeze(tf.image.resize_nearest_neighbor(
            tf.expand_dims(edge, 0),
            new_dim,
            align_corners=True), [0])
...
def resize_to_range(image,
    ...
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if edge is not None:
            if label_layout_is_chw:
                resize_edge = tf.expand_dims(edge, 3)
                resize_edge = tf.image.resize_nearest_neighbor(
                    resize_edge, new_size, align_corners=align_corners)
                resize_edge = tf.squeeze(resize_edge, 3)
            else:
                resize_edge = tf.image.resize_images(
                    edge, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
                    align_corners=align_corners)
            new_tensor_list.append(resize_edge)
        else:
            new_tensor_list.append(None)
